  ...
  $("#"+formid+'').validate({
    ...
    errorPlacement: function(error, element) {
            if (element.attr("name") == "supplier" )
                error.insertAfter(".suppliererror");              
...
function fetchsuppliera(value)
{
    $.ajax({
        url:AdminUrl+"/Stockmaster/SupplierList",
        data:{'value':value},
        type:'POST',
        success:function(result)
        {
            ...
            if(response.status) {
                ...
                $('#supplier').append('<option value="' + response.data[i].id + '">' + response.data[i].name + '</option>');
                });
            }
